...
\begin{figure}
\centering
\includegraphics[width=.5\textwidth]{temp-fig.jpeg}
\caption{The patient's brain activity effectively passes through several filters before an MRI scanner produces a visually interpretable image sequence.}
...
%At the patient level, several confounds can be caused by patient motion.
...
%Next, we examine the physiological properties of the patient. Different tissue types have different material properties and MRI scanners are designed to take advantage of this fact. Changing the TE and TR parameters of a sequence results in the scanner detecting information generated by different tissue types. For example, there are three common types of structural neurological MRIs: T1-weighted images, T2-weighted images, and fluid attenuated inversion recovery (FLAIR) images. The TE and TR times are both short when acquiring T1-weighted images, both long when acquiring T2-weighted images, and both very long when acquiring FLAIR images. T1-weighted images ... tissues with more ELEPHANTS property have stronger signals (brighter voxel values) than tissues with more ELEPHANTS property. The inverse is true for T2-weighted images.

%While MRIs can use the material properties of different tissue types to record information about different organs and organ systems, these material properties can also obscure the BOLD signal.  

%Tissue properties may also vary depending on the patient's stage of life. It is a well-known phenomenon that the material properties of the white and gray matter in brains of newborn patients are the inverse of the material properties of these tissues in patients over 12 months of age. 
...
%Even though the recorded image sequence is not granular enough to express micron-level positional changes
...
% Paper calls them field maps, but are they actually susceptibility maps?
...
%Filtering, which is also referred to as censoring, involves the identification and removal or interpolation of volumes containing high quantities of motion. 
Three popular filtering techniques are scrubbing, spike regression, and despiking. 

During scrubbing, the image sequence is examined for ``high-motion'' volumes. Here, ``high-motion'' is defined as image volumes which have 0.5 mm FD and 0.5\% DVARS difference from their preceeding volume. Volumes containing at least this quantity of motion as well as the preceeding volume and two subsequent volumes are removed from the sequence. As long as the remaining volumes compose a sequence at least 125 frames (approximately 5 minutes) in length, the scrubbed sequence can be used for analysis \cite{Power2012}. Unfortunately, scrubbing results in the loss of data as volumes are removed from the sequence.

Spike regression also identifies volumes containing large quantities of motion \cite{Satterthwaite2013}.

identifies volumes with large FD and replaces them with interpolated volumes . 

Despiking is different from these techniques. It treats the image sequence as a single image volume where each voxel contains an intensity signal. Each intensity signal is examined for sudden changes, or spikes. The value of each detected spike is replaced with an interpolated value calculated using the preceeding and subsequent points in the voxel's signal \cite{Jo2013} \cite{Patel2014}. Despiking does not remove volumes, but could accidentally remove valuable signals if they appear as outliers in the image voxel signals.
